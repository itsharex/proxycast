...
type TabKey = "telegram" | "discord" | "feishu" | "wechat";
...
function WechatGatewayDebugPanel({
  ...
  return (
    <div className={PANEL_CLASS_NAME}>
      <div>
        <h3 className="text-sm font-medium">微信 Gateway 调试工具</h3>
        <p className="text-xs text-muted-foreground">
          支持扫码登录、账号探测、启停控制与已配置账号清理。
        </p>
      </div>
          ...
          <button
            ...
            onClick={() =>
              void runAction(
                "probe",
                async () =>
                  wechatChannelProbe({ accountId: resolveAccountId() }),
                "微信账号探测完成",
              )
            }
            disabled={busy}
      ...
      <div className={`${SOFT_CARD_CLASS_NAME} space-y-3`}>
        <div>
          <h4 className="text-sm font-medium text-slate-900">扫码登录</h4>
          <p className="mt-1 text-xs leading-5 text-slate-500">
            先生成二维码，再等待登录完成。成功后账号会自动写回当前配置。
          </p>
        </div>
      ...
      <div className={`${SOFT_CARD_CLASS_NAME} space-y-3`}>
        ...
        {accounts.length > 0 ? (
          ...
          <p className="text-xs text-slate-500">
            还没有微信账号。可以先去“扫码登录”生成二维码，成功后这里会自动出现。
          </p>
        )}
      </div>
...
export function ChannelsSettings({ className }: ChannelsSettingsProps) {
  ...
  const channelOverview: Array<{
    ...
    },
    {
      key: "wechat",
      label: "微信",
      enabled: channels.wechat.enabled,
      model: channels.wechat.default_model || "跟随全局默认",
    },
  ];
  ...
  const subPages: Array<{
    ...
    },
    {
      key: "config",
      label: "渠道配置",
      description: "维护 Telegram / Discord / 飞书 / 微信基础参数",
            ...
            <div className="max-w-3xl space-y-5">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full border border-white/90 bg-white/88 px-3 py-1.5 text-xs font-medium text-slate-600 shadow-sm">
                  Telegram / Discord / 飞书 / 微信共用同一页治理流程
                </span>
                <span className="rounded-full border border-white/90 bg-white/88 px-3 py-1.5 text-xs font-medium text-slate-600 shadow-sm">
            ...
            >
              <div className="space-y-3">
                ...
                <div className={SOFT_CARD_CLASS_NAME}>
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-500">
                    需要外部回调时，再进入“网关与隧道”处理 tunnel、webhook
                    和回调同步。
                  </p>
                </div>
                <div className={SOFT_CARD_CLASS_NAME}>
              ...
              >
                ...
                <TabsContent value="telegram" className="mt-5 space-y-4">
                  ...
                  <TelegramForm
                    ...
                    onChange={(tg) =>
                      setChannels({ ...channels, telegram: tg })
                    }
                  />
                </TabsContent>
                ...
                <TabsContent value="wechat" className="mt-5 space-y-4">
                  <ConfigGuideCard
                    title="微信配置说明"
                    steps={[
                      "优先在下面的扫码面板里生成二维码，成功后账号会自动写回账号目录。",
                      "如需兼容旧单账号配置，可直接填写全局 Bot Token / Base URL / CDN URL。",
                      "保存后再到“日志与调试”执行探测和启停，确认消息轮询已正常运行。",
                    ]}
                    note="微信模块已独立放在 Rust 目录下；这里仅维护连接参数与账号映射，不依赖 sidecar。"
                  />
                  ...
                  <WechatForm
                    config={channels.wechat}
                    onChange={(wc) => setChannels({ ...channels, wechat: wc })}
                  />
                </TabsContent>
              </Tabs>
            </div>
      ...
      {activeSubPage === "logs" && (
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.02fr)_minmax(0,0.98fr)]">
          <SurfacePanel
            icon={ScrollText}
            title="渠道日志观察"
            description="支持实时 tail、过滤与清空，快速定位渠道网关 / RPC 问题。"
          >
            <div className="space-y-4">
              <ConfigGuideCard
                title="日志排查说明"
